...
class baseTrainer:
    ...
    def __init_model(self):
        ...
        if self.model_type == "MAE_ViT":
            self.model = MAE_Pretrain_Baseline(
                image_size=(224//self.downsample, 192//self.downsample, 224//self.downsample),
                in_channels=in_channel
            ) 

            print(self.model)
        elif self.model_type == "MAE_ViT_Hybrid":
            self.model = MAE_Pretrain_HybridNet(
                image_size=(224//self.downsample, 192//self.downsample, 224//self.downsample),
                in_channels=in_channel
            )
            print(self.model)
...
class Trainer(baseTrainer):
            ...
            for batch_idx, batch_data in enumerate(train_loader):
                ...
                if self.scaler:
                    with torch.cuda.amp.autocast():
                        dec_img, recon_patch, orig_patch = self.model(model_input)
    ...
    @staticmethod
    def __compute_loss(
        loss_fn, fixed_img, moving_img, mode="train", downsample=1
    ):
        loss = 0.0
        all_loss = {}
        for l, lw in loss_fn.items():
            if l == "NCC":
                ncc_loss = lw[0](fixed_img, moving_img) * lw[1]
                loss += ncc_loss
                all_loss["NCC"] = ncc_loss.item()
            elif l == "GNCC":
                gncc_loss = lw[0](fixed_img, moving_img) * lw[1]
                loss += gncc_loss
                all_loss["GNCC"] = gncc_loss.item()
            elif l == "MSE":
                mse_loss = lw[0](fixed_img, moving_img) * lw[1]
                loss += mse_loss
                all_loss["MSE"] = mse_loss.item()
            elif l == "SAD":
                sad_loss = lw[0](fixed_img, moving_img) * lw[1]
                loss += sad_loss
                all_loss["SAD"] = sad_loss.item()
            elif l == "MINDSSC" or l == "MINDSSC_NCC":
                mindssc_loss = lw[0](fixed_img, moving_img) * lw[1]
                loss += mindssc_loss
