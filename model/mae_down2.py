...
class MAE(nn.Module):
    def __init__(
        ...
        #self.conv3d_transpose = nn.ConvTranspose3d(decoder_dim, channels, kernel_size=new_patch_size, stride=new_patch_size)
...
class MAE_Finetune(nn.Module):
    def __init__(
        ...
        self.mae_transformer1 = MAE(
            image_size=(image_size[0]//2**self.down_factor, image_size[1]//2**self.down_factor, image_size[2]//2**self.down_factor),
            patch_size=16//2**self.down_factor,
            encoder_dim=768,
            mlp_dim=3072,
    ...
    def forward(self, x, y):
        ...
        #print(f"y:{y.shape}")
        x = F.interpolate(x, scale_factor=1/2**self.down_factor, mode='trilinear', align_corners=False)
        y = F.interpolate(y, scale_factor=1/2**self.down_factor, mode='trilinear', align_corners=False)
        #print(f"dsx:{x.shape}")
        #print(f"dsy:{y.shape}")
        x1 = self.mae_transformer1(x)
        x2 = self.mae_transformer2(y)
        #print(f"x1:{x1.shape}")
        #print(f"x2:{x2.shape}")
        x1 = F.interpolate(x1, scale_factor=2**self.down_factor, mode='trilinear', align_corners=False)
        x2 = F.interpolate(x2, scale_factor=2**self.down_factor, mode='trilinear', align_corners=False)
