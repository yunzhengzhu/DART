...
class UNet(nn.Module):
    def __init__(self, in_channel=2, n_classes=3, kernel="regular", kernel_dec="regular", lknorm="batchnorm"):
        ...
        channel_factor = 8
        if kernel == "regular":
            self.encoder = nn.ModuleDict({'enc1':ConvBlock(in_channel,64*16//channel_factor),\
                                          'enc2':ConvBlock(64*16//channel_factor,128*16//channel_factor),\
                                          'enc3':ConvBlock(128*16//channel_factor,256*16//2//channel_factor),\
                                          'enc4':ConvBlock(256*16//2//channel_factor,512*16//4//channel_factor),\
                                          'enc5':ConvBlock(512*16//4//channel_factor,1024*16//8//channel_factor)}) #,\
        elif kernel == "large":
            bias_opt = False
        ...
        if kernel_dec == "regular":
            ...
            bias_opt = False
            self.decoder = nn.ModuleDict({'dec1':ConvBlock(1024*16//8//channel_factor+512*16//4//channel_factor,512*16//4//channel_factor),\
                                          'dec2':LK_encoder(512*16//4//channel_factor,512*16//4//channel_factor,norm=lknorm,bias=bias_opt),\
                                          'dec3':ConvBlock(512*16//4//channel_factor+256*16//2//channel_factor,256*16//2//channel_factor),\
                                          'dec4':LK_encoder(256*16//2//channel_factor,256*16//2//channel_factor,norm=lknorm,bias=bias_opt),\
                                          'dec5':ConvBlock(256*16//2//channel_factor+128*16//channel_factor,128*16//channel_factor),\
                                          'dec6':LK_encoder(128*16//channel_factor,128*16//channel_factor,norm=lknorm,bias=bias_opt),\
                                          'dec7':ConvBlock(128*16//channel_factor+64*16//channel_factor,64*16//channel_factor),\
                                          'dec8':LK_encoder(64*16//channel_factor,64*16//channel_factor,norm=lknorm,bias=bias_opt)})
...
class AttentionUNet(nn.Module):
    def __init__(self, in_channel=2, n_classes=3, kernel="regular", kernel_dec="regular", lknorm="batchnorm"):
        ...
        self.kernel_dec = kernel_dec
        if kernel_dec == "regular":
            self.decoder = nn.ModuleDict({'att1':AttentionBlock(1024*16//8//channel_factor,512*16//4//channel_factor,512*16//4//channel_factor),\
                                          'dec1':ConvBlock(1024*16//8//channel_factor+512*16//4//channel_factor,512*16//4//channel_factor),\
                                          'att2':AttentionBlock(512*16//4//channel_factor,256*16//2//channel_factor,256*16//2//channel_factor),\
                                          'dec2':ConvBlock(512*16//4//channel_factor+256*16//2//channel_factor,256*16//2//channel_factor),\
                                          'att3':AttentionBlock(256*16//2//channel_factor,128*16//channel_factor,128*16//channel_factor),\
                                          'dec3':ConvBlock(256*16//2//channel_factor+128*16//channel_factor,128*16//channel_factor),\
                                          'att4':AttentionBlock(128*16//channel_factor,64*16//channel_factor,64*16//channel_factor),\
                                          'dec4':ConvBlock(128*16//channel_factor+64*16//channel_factor,64*16//channel_factor)})
        elif kernel_dec == "large":
            bias_opt = False
...
class AttentionAugUNet(nn.Module):
    def __init__(self, in_channel=2, n_classes=3, kernel="regular", kernel_dec="regular", lknorm="batchnorm", image_size=(224, 192, 224)):
        '''
        kernel: large for lkunet, regular for unet
        '''
        super().__init__()
        ...
        if self.kernel == "regular":
            ...
            for i in range(1,9):
                ...
                if i % 2 == 0 and i < 7:
                    y.append(x)
                    x = F.max_pool3d(x,2)
        x = self.encoder['encattaug'](x)
        #x = self.encoder['attaug'](x)
        if self.kernel_dec == "regular":
            for i in range(4):
                # upsample + conv
                x = torch.cat((upsample(x),y.pop()),1)
...
class PyramidUNet(nn.Module):
    def __init__(self, in_channel=2, n_classes=3, kernel="regular", kernel_dec="regular", lknorm="batchnorm"):
        ...
        self.enc1 = ConvBlock(in_channel,64*16//channel_factor)
    ...
    def forward(self, x):
        ...
        if self.kernel == "regular":
            # first pyramid
            x_p1 = self.enc1(x)
            x_p1 = F.max_pool3d(x_p1, 2)

            # second pyramid
            x_2 = F.interpolate(x, align_corners=True, scale_factor=2, mode='trilinear')
            x_p2 = self.enc2(x_p2)
            
            # cat first & second pyramid
            x_p12 = torch.cat((x_p1, x_p2), dim=1)

            # third pyramid
            x_p3 = F.interpolate(x_2, align_corners=True, scale_factor=2, mode='trilinear')
    ...
    def __init__(
        ...
        if model_type == "regular":
            ...
            self.unet = AttentionUNet(in_channel=in_channel, 
                                      ...
                                      kernel_dec=kernel_dec,
                                      lknorm=lknorm)
        elif model_type == "attentionaug":
            self.unet = AttentionAugUNet(in_channel=in_channel, 
                                         n_classes=n_classes, 
                                         kernel=kernel, 
                                         kernel_dec=kernel_dec, 
                                         lknorm=lknorm)
