...
class UNet(nn.Module):
    def __init__(self, in_channel=2, n_classes=3, kernel="regular", lknorm="batchnorm"):
        ...
        self.decoder = nn.ModuleDict(
            {
                "dec1": ConvBlock(
                    512 // channel_factor + 256 // channel_factor, 256 // channel_factor
                ),
                "dec2": ConvBlock(
                    256 // channel_factor + 128 // channel_factor, 128 // channel_factor
                ),
                "dec3": ConvBlock(
                    128 // channel_factor + 64 // channel_factor, 64 // channel_factor
                ),
                "dec4": ConvBlock(
                    64 // channel_factor + 32 // channel_factor, 32 // channel_factor
                ),
            }
        )
        self.conv1 = ConvBlock(32 // channel_factor, 64 // channel_factor)
        self.conv2 = nn.Sequential(
            nn.Conv3d(64 // channel_factor, 32 // channel_factor, 1, bias=False),
            nn.InstanceNorm3d(32 // channel_factor),
            nn.ReLU(inplace=True),
            nn.Conv3d(32 // channel_factor, 32 // channel_factor, 1, bias=False),
            nn.InstanceNorm3d(32 // channel_factor),
            nn.ReLU(inplace=True),
...
class UNetReg(nn.Module):
    def __init__(self, in_channel=2, n_classes=3, kernel="regular", lknorm="batchnorm"):
        super().__init__()
        self.unet = UNet(
            in_channel=in_channel, n_classes=n_classes, kernel=kernel, lknorm=lknorm
        )
