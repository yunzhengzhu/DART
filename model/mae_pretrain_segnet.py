...
class MAE(nn.Module):
    def __init__(
        ...
        #self.conv3d_transpose1 = nn.ConvTranspose3d(
        ...
        #self.conv3d_transpose_seg1 = nn.ConvTranspose3d(
        #    in_channels=16, out_channels=num_masks, kernel_size=new_patch_size, stride=new_patch_size
        #)
    ...
    def forward(self, img):
        ...
        #x_recon = self.conv3d_transpose1(x_recon) # [B, out_c(1), cr * (stride(4) ** 2), cr * (stride(4) ** 2), cr * (stride(4) ** 2)]


        # seg head
        x_seg = self.conv3d_transpose_seg(x)
        #x_seg = self.conv3d_transpose_seg1(x_seg) # [B, out_c(1), cr * (stride(4) ** 2), cr * (stride(4) ** 2), cr * (stride(4) ** 2)]
...
class MAE_Pretrain_HybridSegNet(nn.Module):
    def __init__(
        self,
        image_size=(112, 96, 112),
        in_channels=1,
    ...
    def forward(self, x):
        ...
        #print(f"output_recon:{output_recon.shape}")
        #print(f"output_seg:{output_seg.shape}")
        output_seg = F.interpolate(output_seg, scale_factor=2**self.down_factor, mode='trilinear', align_corners=False)
