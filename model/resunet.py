    ...
    def __init__(self, in_channel=2):
        ...
        for name, module in self.resnet.named_modules():
            if isinstance(module, nn.Conv2d):
                ...
                set_layer(self.resnet, name, after)
                count += 1
...
class UNet(nn.Module):
    def __init__(
        self, in_channel=256, n_classes=3, kernel="regular", lknorm="instancenorm"
    ):
        super().__init__()
        self.kernel = kernel
        channel_factor = 1
        if kernel == "regular":
            self.encoder = nn.ModuleDict(
                {
                    "enc1": ConvBlock(in_channel, 32 // channel_factor),
            ...
            self.encoder = nn.ModuleDict(
                {
                    "enc1": ConvBlock(256, int(32 // channel_factor)),
                    "enc2": ConvBlock(
                        int(32 // channel_factor), int(64 // channel_factor)
                    ),
                    "enc3": LK_encoder(
                        int(64 // channel_factor),
                        int(64 // channel_factor),
                        norm=lknorm,
                        bias=bias_opt,
                    ),
                    "enc4": ConvBlock(
                        int(64 // channel_factor), int(128 // channel_factor)
                    ),
                    "enc5": LK_encoder(
                        int(128 // channel_factor),
                        int(128 // channel_factor),
                        norm=lknorm,
                        bias=bias_opt,
                    ),
                    "enc6": ConvBlock(
                        int(128 // channel_factor), int(256 // channel_factor)
                    ),
                    "enc7": LK_encoder(
                        int(256 // channel_factor),
                        int(256 // channel_factor),
                        norm=lknorm,
                        bias=bias_opt,
                    ),
        ...
        self.decoder = nn.ModuleDict(
            {
                "dec1": ConvBlock(
                    64 // channel_factor + 48 // channel_factor, 48 // channel_factor
                ),
                "dec2": ConvBlock(
                    48 // channel_factor + 48 // channel_factor, 48 // channel_factor
                ),
                "dec3": ConvBlock(
                    48 // channel_factor + 32 // channel_factor, 32 // channel_factor
                ),
            }
        )
        ...
        self.conv2 = nn.Sequential(
            nn.Conv3d(
                int(64 // channel_factor), int(32 // channel_factor), 1, bias=False
            ),
            nn.InstanceNorm3d(int(32 // channel_factor)),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                int(32 // channel_factor), int(32 // channel_factor), 1, bias=False
            ),
            nn.InstanceNorm3d(int(32 // channel_factor)),
            nn.ReLU(inplace=True),
...
class ResUNetReg(nn.Module):
    def __init__(
        self, in_channel=2, n_classes=3, kernel="regular", lknorm="instancenorm"
    ):
        super().__init__()
        self.resnet = ResNet(in_channel=in_channel)
        self.unet = UNet(
            in_channel=256, n_classes=n_classes, kernel=kernel, lknorm=lknorm
        )

    def forward(self, x, y):
        input = torch.cat(
            (self.resnet(x.to(torch.float)), self.resnet(y.to(torch.float))), 1
        )
